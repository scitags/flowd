...
FLOW_IDS = [(2, 19), (2, 19), (3, 3), (3, 3), (4, 3), (4, 3), (5, 10), (5,10)]
# FLOW_IPERF = { '<ip6>': (3, 3), '<ip6>': (5, 10), '<ip6>': (4, 3), '<ip6>': (2, 19) }
RR_FLOW_IDS = itertools.cycle(FLOW_IDS)
...
def run(flow_queue, term_event, ip_config):
    ...
    while not term_event.is_set():
        ...
        if init_pass:
            ...
            for k, v in netstat.items():
                ...
                if __int_ip(daddr, int_networks):
                    continue
                if type(daddr) is ipaddress.IPv4Address:
                    continue
        ...
        for k, v in netstat.items():
            ...
            #if type(daddr) is ipaddress.IPv4Address:
            #    continue
            if not 5200 < k[2] < 5300: # only iperf3 source port range
                continue
            if k not in netstat_index.keys() and v == 'ESTABLISHED':
                ...
                exp, act = next(RR_FLOW_IDS)
                #if k[3] in FLOW_IPERF.keys():
                #    exp, act = FLOW_IPERF[k[3]]
                #else:
                #    exp, act = next(RR_FLOW_IDS)
